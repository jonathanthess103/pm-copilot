...
- **Stage 2 - Standing initiatives.** If the user's own memory overrides this concept (per Step 2.7), present its status check in that file's own terms instead, and draft the short current-state line that file expects for each item it tracks, so Step 5 has something to write back. Otherwise: present the proposed active set, each with its one-line "why now." Form: confirm / swap / mark a different one active / propose new.
...
- Update the standing initiatives: if the user's own memory overrides this concept, write the confirmed current-state lines from Stage 2 back into that file, following its own rules for where each line goes and whether it is overwritten or appended. This write is not optional. A run that collects those lines and never writes them leaves the next run reading a stale note as if it were current, which is worse than carrying no note at all. Otherwise set active/dormant, write the confirmed "why now" lines, link tasks, create any confirmed new initiative.
